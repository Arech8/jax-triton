...
def get_recommended_splits(num_sequences, num_kv_heads):
  num_sm = jax.devices()[0].core_count * get_occupancy()
  max_context_partition_num = min(16, triton.cdiv(num_sm, num_sequences * num_kv_heads))
...
def paged_attention_decode_v2_gluon_large_block_dot_kernel(
  ...
  mtp_query_len_offsets = gl.arange(0, QUERY_SEQ_LEN_POW2, layout=mtp_query_len_layout)
  ...
  if QUERY_QUANT_MODE == 0:
    ...
    query_scale_value = gl.convert_layout(query_scale_value, layout=qk_linear_layout)
  ...
  max_logits_group_idx_in_len = max_logits_base_offsets_mtp % ONE_QUERY_GROUP_SIZE_POW2
  ...
  KV_COMPUTE_BLOCK_COUNT: gl.constexpr = CONTEXT_PARTITION_SIZE // KV_COMPUTE_BLOCK_SIZE
  ...
  for kv_block_index in range(KV_COMPUTE_BLOCK_COUNT):
    ...
    if VALUE_TRANSPOSED:
      ...
      value_block_offsets = (
        ...
        + (current_page_offset // CONTIGUOUS_KV_ELEMENTS_16B_LOAD + value_dim0_offsets)[
          :, None, None
        ]
        * stride_value_head_size
    ...
    if value_block.dtype.is_fp8():
      if KV_QUANT_MODE == 1:
        ...
        value_scale_value = tl.where(valid_token_mask, value_scale_value, float(0.0))
...
def paged_attention_decode_sliding_window(
  ...
  MAX_NUM_KV_BLOCKS_PER_COMPUTE: gl.constexpr = CONTEXT_PARTITION_SIZE // KV_BLOCK_SIZE
  ...
  if SLIDING_WINDOW > 0:
    ...
    sequence_end_idx = gl.minimum(context_length, page_size * (sequence_split_idx + 1))
  ...
  for sequence_partition_idx in range(
    ...
    if VALUE_TRANSPOSED:
      ...
      value_tensor = gl.reshape(value_tensor, [CONTEXT_PARTITION_SIZE, HEAD_SIZE_POW2])
    else:
    ...
    if KV_QUANT_MODE >= 0:
      if KV_QUANT_MODE == 1:
        ...
        valid_token_mask = qk_column_offsets < context_length
        # Mask out value_scale of invalid tokens
        value_scale_value = gl.where(valid_token_mask, value_scale_value, float(0.0))
        value_scale_max = gl.max(value_scale_value, axis=0)
        # Scale the maximum value of value_scale to FP8_MAX_VALUE to improve the precision of P * V
        value_scale_value = (
          value_scale_value * float(FP8_MAX_VALUE) / (value_scale_max + 1e-8)
...
def paged_attention_decode_v2_reduce_kernel(
  ...
  QUERY_GROUP_SIZE_POW2: gl.constexpr = OUTPUT_SEQ_LEN_POW2 * ONE_OUTPUT_GROUP_SIZE_POW2
...
def _paged_attention_decode_v2_reduce_kernel_wrapper(
  ...
  if PS:
    paged_attention_decode_ps_reduce_kernel[grid](
      ...
      MAX_CONTEXT_PARTITION_NUM=min(triton.next_power_of_2(context_partition_num), 16),
    )
  else:
...
def pa_decode_gluon(
  ...
  if query_scale is not None:
    assert (
      isinstance(query_scale, torch.Tensor) and query_scale.dtype == aiter.dtypes.fp32
    ), (
  ...
  if (
    ...
    assert (
      isinstance(value_scale, torch.Tensor) and value_scale.dtype == aiter.dtypes.fp32
    ), (
